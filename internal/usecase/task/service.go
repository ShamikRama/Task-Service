...
func (s *Service) Update(ctx context.Context, id int64, input UpdateInput) (*taskdomain.Task, error) {
	...
	model := *existing
	model.Title = normalized.Title
	model.Description = normalized.Description
	model.Status = normalized.Status
	if input.UnlinkTemplate {
		...
		model.TemplateID = &tid
	}
	if input.ClearScheduledDate {
		model.ScheduledDate = nil
	} else if input.ScheduledDate != nil {
		sd := *input.ScheduledDate
		model.ScheduledDate = &sd
	}
